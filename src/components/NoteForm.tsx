...
type Level = "year" | "semester" | "subject" | "chapter" | "topic";
...
export function NoteForm({ note, defaults }: Props) {
  ...
  const [topicId, setTopicId] = useState(note?.topicId ?? defaults?.topic ?? "");
  ...
  const semesterOptions = useMemo(
    () => (semesters.data ?? []).filter((s) => s.yearId === yearId),
    [semesters.data, yearId],
  );
  const subjectOptions = useMemo(
    () => (subjects.data ?? []).filter((s) => s.semesterId === semesterId),
    [subjects.data, semesterId],
  );
  const chapterOptions = useMemo(
    () => (chapters.data ?? []).filter((c) => c.subjectId === subjectId),
    [chapters.data, subjectId],
  );
  const topicOptions = useMemo(
    () => (topics.data ?? []).filter((t) => t.chapterId === chapterId),
    [topics.data, chapterId],
  );
  ...
  async function handleCreateEntity(level: Level, name: string) {
    ...
    if (level === "year") {
      ...
      const id = await api.createEntity(uid, "semesters", {
        name,
        yearId,
        order: semesterOptions.length + 1,
      });
      await invalidate();
      setSemesterId(id);
      setSubjectId("");
      ...
      const id = await api.createEntity(uid, "chapters", {
        name,
        yearId,
        semesterId,
        subjectId,
        order: chapterOptions.length + 1,
      });
      await invalidate();
      setChapterId(id);
      setTopicId("");
    } else {
      const id = await api.createEntity(uid, "topics", {
        name,
        yearId,
        semesterId,
        subjectId,
  ...
  async function handleSave() {
    ...
    if (!yearId || !semesterId || !subjectId || !chapterId || !topicId) {
      toast.error("Choose a year, semester, subject, chapter and topic.");
      return;
    }
    ...
    const payload = {
      ...
      plainText,
      yearId,
      yearName: nameOf(years.data, yearId),
      semesterId,
      semesterName: nameOf(semesters.data, semesterId),
      subjectId,
      subjectName: nameOf(subjects.data, subjectId),
      chapterId,
      chapterName: nameOf(chapters.data, chapterId),
      topicId,
      topicName: nameOf(topics.data, topicId),
      attachments,
    };
    ...
    try {
      if (note) {
        await api.updateNote(uid, note.id, payload);
        await invalidate();
        toast.success("Note updated successfully.");
        navigate({ to: "/notes/$id", params: { id: note.id } });
      } else {
        const id = await api.createNote(uid, payload);
        await invalidate();
        toast.success("Note created successfully.");
        navigate({ to: "/notes/$id", params: { id } });
      }
    } catch (error) {
      toast.error(friendlyError(error, "Something went wrong while saving your note."));
    } finally {
      setSaving(false);
    }
  }
    ...
    <div className="space-y-6">
      <div className="space-y-2">
        ...
        <Input
          id="note-title"
          value={title}
          placeholder="e.g. Binary Search Trees — traversals"
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        {levels.map((item) => (
          <div key={item.level} className="space-y-2">
            <Label htmlFor={`select-${item.level}`}>{item.label}</Label>
            <div className="flex gap-1.5">
              ...
              >
                <SelectTrigger id={`select-${item.level}`} className="flex-1">
              ...
              <Button
                ...
                onClick={() => setCreating(item.level)}
              >
      ...
      <div className="space-y-3">
        ...
        {attachments.length ? (
          ...
          <p className="rounded-lg border border-dashed border-border px-4 py-6 text-center text-sm text-muted-foreground">
            No attachments yet. PDFs, slides, spreadsheets, images and archives are all supported.
          </p>
        )}
      </div>

      <div className="flex justify-end gap-2">
        <Button variant="ghost" onClick={() => navigate({ to: "/notes" })} disabled={saving}>
          Cancel
        </Button>
      ...
      <EntityDialog
        ...
        onSubmit={async (name) => {
          if (creating) await handleCreateEntity(creating, name);
        }}
      />
      ...
      <ConfirmDialog
        ...
        onConfirm={async () => {
          ...
          try {
            await api.deleteStoredFile(uid, pendingDelete.fileId, pendingDelete.path);
            setAttachments((prev) => prev.filter((a) => a.fileId !== pendingDelete.fileId));
            if (note) {
              await api.updateNote(uid, note.id, {
                title: note.title,
                content: note.content,
                plainText: note.plainText,
                yearId: note.yearId,
                yearName: note.yearName,
                ...
                attachments: attachments.filter((a) => a.fileId !== pendingDelete.fileId),
              });
            }
            await invalidate();
